...
from flwr_torch_gru.Crypto.fhe_fed_avg import FheFedAvg
from flwr_torch_gru.task import Net, get_weights, set_weights, test
import json
import pandas as pd
...
mlflow.set_experiment("MLflow gru df1")
mlflow.start_run(run_name = "Gobal_flwr-fhe-torch-gru", log_system_metrics=True)
...
def avg_metrics(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    for i,m in metrics:
        ...
        # Log client metric
        mlflow.start_run(run_name = "Client_flwr-fhe-torch-gru", nested=True)
    ...
    mlflow.start_run(run_name = "Client_flwr-fhe-torch-gru_avg", nested=True)
...
def get_eval_func(valloader, g_model, num_rounds, params, Test_ds):
    ...
    def eval(server_round, parameters_ndarrays, config): # server_round == current round
        ...
        if server_round == num_rounds:
            ...
            signature=signature, 
            registered_model_name="Gobal_flwr-fhe-torch-gru_df1", 
