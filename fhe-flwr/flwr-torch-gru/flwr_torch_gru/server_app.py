...
# mlflow.set_experiment("MLflow gru")
...
def avg_metrics(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    for i,m in metrics:
        ...
        # # Log client metric
        # mlflow.start_run(run_name = "Client_flwr-torch-gru", nested=True)
    ...
    # mlflow.start_run(run_name = "Client_flwr-torch-gru_avg", nested=True)
...
def get_eval_func(valloader, g_model, num_rounds, params, Test_ds):
    ...
    def eval(server_round, parameters_ndarrays, config): # server_round == current round
        ...
        #     signature=signature, 
        #     registered_model_name="Gobal_flwr-fhe-torch-gru", 
