...
from flwr_torch_multiheadattention.Crypto.fhe_fed_avg import FheFedAvg
...
mlflow.set_experiment("MLflow MultiheadAttention df2")
mlflow.start_run(run_name = "Gobal_flwr-fhe-torch-MultiheadAttention", log_system_metrics=True)
...
def avg_metrics(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    for _,m in metrics:
        ...
        # Log client metric
        mlflow.start_run(run_name = "Client_flwr-fhe-torch-MultiheadAttention", nested=True)
    ...
    mlflow.start_run(run_name = "Client_flwr-fhe-torch-MultiheadAttention_avg", nested=True)
...
def get_eval_func(valloader, g_model, num_rounds, params, Test_ds):
    ...
    def eval(server_round, parameters_ndarrays, config): # server_round == current round
        ...
        if server_round == num_rounds:
            ...
            signature=signature, 
            registered_model_name="Gobal_flwr-fhe-torch-MultiheadAttention_df2", 
