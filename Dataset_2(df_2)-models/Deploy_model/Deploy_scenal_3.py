...
# Start MLflow run
mlflow.set_experiment("Deploy mhe df2")
with mlflow.start_run(run_name=f"Deploy_scenal_2&3_fhe-mhe"):
    # Enable system metrics logging every 0.1 seconds
    ...
    for size_name, test_df in test_sets:
        ...
        prepared_data[size_name] = (X_test, y_test)

    # Continuous testing for 5 seconds on each data percentage
    for size_name, (X_test, y_test) in prepared_data.items():
        print(f"\nStarting 5-second continuous testing on {size_name}% data...")
        ...
        while time.time() - start_time < 5:
            ...
            wave_end = time.time()
            inference_time = round(wave_end - wave_start, 3)
            ...
            # print(f"Wave {wave_count} - {size_name}: P={precision}, R={recall}, F1={f1_score}, T={inference_time}s, CPU={cpu_after}%, RAM={ram_after:.1f}MB")
            
            # Log each wave separately including system metrics
            mlflow.log_metric(f"wave_time_{size_name}", inference_time, step=wave_count)
            # mlflow.log_metric(f"wave_precision_{size_name}", precision, step=wave_count)
            # mlflow.log_metric(f"wave_recall_{size_name}", recall, step=wave_count)
            # mlflow.log_metric(f"wave_f1_{size_name}", f1_score, step=wave_count)
            mlflow.log_metric(f"wave_cpu_{size_name}", cpu_after, step=wave_count)
            mlflow.log_metric(f"wave_ram_mb_{size_name}", ram_after, step=wave_count)
        
        print(f"Completed {wave_count} waves for {size_name}% data in 5 seconds")