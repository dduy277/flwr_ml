...
from flwr_torch_rnn.Crypto.fhe_fed_avg import FheFedAvg
from flwr_torch_rnn.task import Net, get_weights, set_weights, test
import json
import pandas as pd
...
mlflow.set_experiment("MLflow rnn df2")
mlflow.start_run(run_name = "Gobal_flwr-fhe-torch-rnn", log_system_metrics=True)
...
def avg_metrics(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    for i,m in metrics:
        ...
        # Log client metric
        mlflow.start_run(run_name = "Client_flwr-fhe-torch-rnn", nested=True)
    ...
    mlflow.start_run(run_name = "Client_flwr-fhe-torch-rnn_avg", nested=True)
...
def get_eval_func(valloader, g_model, num_rounds, params, Test_ds):
    ...
    def eval(server_round, parameters_ndarrays, config): # server_round == current round
        ...
        if server_round == num_rounds:
            ...
            signature=signature, 
            registered_model_name="Gobal_flwr-fhe-torch-rnn_df2", 
